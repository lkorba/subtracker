...
function AuthPage() {
  ...
  const handleGoogle = async () => {
    setLoading(true);
    // Supabase's native OAuth flow. The redirect URL must be in the Supabase
    // allow-list AND Google Cloud's authorized redirect URIs list.
  ...
  const handleEmail = async (e: React.FormEvent<HTMLFormElement>, mode: "in" | "up") => {
    ...
    if (mode === "up")
      toast.success("Check your email to confirm — or sign in if already confirmed.");
...
function GoogleIcon() {
  return (
    <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24">
      ...
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.2 1.64l3.15-3.15C17.45 2.09 14.96 1 12 1A11 11 0 0 0 2.18 7.07l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38Z"
      />
    </svg>
  );
}
